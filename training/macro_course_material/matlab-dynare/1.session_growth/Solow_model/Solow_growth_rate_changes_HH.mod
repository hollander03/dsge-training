...
@# define SCENARIO = 2  // Set 1 for g-shock, 2 for n-shock
...
@#if (SCENARIO == 1)
// Simulation with change in technology growth (g)
initval;
    g=g_initial;
    n=n_initial;
...
@#if (SCENARIO == 2)
// Simulation with change in population growth (n)
initval;
    g=g_initial;
    n=n_initial;
    k=((delta+n+g_initial+n*g_initial)/s)^(1/(alpha-1));
...
var_index = strmatch('log_k', M_.endo_names, 'exact');
log_k_sim = oo_.endo_simul(var_index,:)';

var_index = strmatch('g_k_intensive', M_.endo_names, 'exact');
g_k_intensive_sim = oo_.endo_simul(var_index,:)';

var_index = strmatch('g_k_per_capita', M_.endo_names, 'exact');
g_k_per_capita_sim = oo_.endo_simul(var_index,:)';

var_index = strmatch('g_k_aggregate', M_.endo_names, 'exact');
g_k_aggregate_sim = oo_.endo_simul(var_index,:)';
