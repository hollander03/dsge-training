...
// NOTE: eb shock adjusted to reflect paper - see "0*b" in euler eqn and capital price eqns for actual and flex price equil
...
// flexible economy
          ...
          pkf = -rrf - b + 0*(1/((1-chabb/cgamma)/(csigma*(1+chabb/cgamma))))*b +(crk/(crk+(1-ctou)))*rkf(1) +  ((1-ctou)/(crk+(1-ctou)))*pkf(1) ;
          //consumption Euler equation
	      cf = (chabb/cgamma)/(1+chabb/cgamma)*cf(-1) + (1/(1+chabb/cgamma))*cf(+1) +((csigma-1)*cwhlc/(csigma*(1+chabb/cgamma)))*(labf-labf(+1)) - (1-chabb/cgamma)/(csigma*(1+chabb/cgamma))*(rrf+b) + 0*b ;
...
// sticky price - wage economy
          ...
          pk = -r+pinf(1)-b + 0*(1/((1-chabb/cgamma)/(csigma*(1+chabb/cgamma))))*b + (crk/(crk+(1-ctou)))*rk(1) +  ((1-ctou)/(crk+(1-ctou)))*pk(1) ;
          //consumption Euler equation
...
stoch_simul(order=1, irf=20, graph_format=fig) y pinf r outputgap; %  r pinf lab y