...
varexo ea eb eg eqs em epinf ew et 

//**************************************************************************
// Modelbase Shocks                                                    
  fiscal_  ;       //interest_     dummy_MP   dummy_G                                   
//**************************************************************************
...
parameters curvw cgy curvp constelab constepinf constebeta cmaw cmap calfa 
           ...
           cindp cprobp csigl clandaw crdpi crpi crdy cry crr crhoa crhob crhot 
...
// sticky price - wage economy
          ...
          // (39) NT-DSGE Non-Ricardian consumption
          %(1+tauCbar)*(tauCbar/(1+tauCbar)*tauC + c_lc) = cwhlc*(w + lab) - 1/ccy*t;
          ...
          % NT-DSGE VAT included
          %debt = cr*(1/cpie*debt(-1) + g - t - (tauC + tauCbar*(c - y)));

          //fiscal rule
          t = phi_b*debt(-1) + phi_g*g + crhot*(t(-1)) + et;
          ...
          // g = fiscal_;
	      g = crhog*(g(-1)) + fiscal_ + eg; // + phi_t*t
	     //g = crhog*(g(-1))  + cgy*ea;
...
stoch_simul(order=1, irf=20, graph_format=fig)  output inflationq interest outputgap ;  // debtf tf
