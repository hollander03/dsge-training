...
bstar               $\hat{b}^*$             % Foreign government debt / foreign bonds
...
epsilonN            $\hat{\varepsilon}^i$       % Labour supply/disutility shock
...
chistar         =   0.2; %0.391749013; %0.2;
thetaW          =   0.75;
thetaH          =   0.8; %0.80741582; %0.9;
thetaX          =   0.7; %0.692197722; %0.6;
thetastar       =   0.75;  %0.786893185; %0.7;   
varphiWbar      =   1.05; 
varphiHbar      =   1.1; 
sigmaL          =   5;
omega           =   0.5;  // 232525868373284
...
alphaK_target       =   0.9;
cJcISSratio         =   0.8;
tr2rot              =   0.5; // or 0.75 calibrated to DSGE paper est
sIMC                =   0.224;    
sIMI                =   0.06;
sI                  =   0.126;
sG                  =   0.195;
...
lambda        =   lambda(+1) - gz(+1) + epsilonRPG + r - piC(+1);

%(5) UIP 
epsilonRPG + r - rstar  =   s(+1) - s + piY(+1) - piYstar(+1)
                            - gammaBstar*lamRPstar*sbar/ybar*bstar + epsilonRPstar; // lamRPstar*
...
w             =   betta/(1+betta)*w(+1) + 1/(1+betta)*w(-1) + betta/(1+betta)*piC(+1) 
                           ...
                           ( w - (1/(1-tauNbar))*(tauN)  - (epsilonN + sigmaL*N - lambda) ) + varphiW; 
...
%(16) Import Phillips %% CHECK: *(s + pY - pIM)
piIM - piCbar   =   betastar/(1+betastar*chistar)*(piIM(+1) - piCbar(+1)) + chistar/(1+betastar*chistar)*(piIM(-1) - piCbar)
                       + betastar*chistar/(1+betastar*chistar)*(piCbar(+1) - piCbar)
                       + (1-betastar*thetastar)*(1-thetastar)/(thetastar*(1+betastar*chistar))*(s + pY - pIM) + varphistar;
...
%(23) Taylor rule
r             =   phiR*r(-1)+ (1-phiR)*(piCbar + phiPi*(piC - piCbar) + phiY*y)
                          + phiDPi*(piC - piC(-1))+ phiDY*(y - y(-1)) + etaR; //piC(-1) phiY*y(-1)
...
%(32) Government / fiscal budget constraint
...
%(42) Domestic risk premium shock
epsilonRPG    =   rhoRPG*epsilonRPG(-1) + etaRPG;

%(43) Foreign risk premium shock // CHECK: function of debt?
epsilonRPstar =   rhoRPstar*epsilonRPstar(-1) + etaRPstar;
...
piYstar        =       betastar*piYstar(+1) + ((1-thetastar)*(1-thetastar*betastar))/(thetastar)*((sigmastar + sigmastarn)*ystar - (1 + sigmastarn)*epsilonYstar); // mcstar subst out
%piYstar        =       betastar*piYstar(+1) + ((1-thetastar)*(1-thetastar*betastar))/(thetastar)*(mcstar); // + xi_p_star foreign Phillips Curve 
...
ystar        =           ystar(+1) - (1/sigmastar)*(rstar - piYstar(+1) + epsilonRPstar);  //   foreign IS curve

%(59) Foreign interest rate
%rstar         =         rhorstar*rstar(-1) + (1 - rhorstar)*(kappapistar*piYstar + kappaystar*ystar) + etaRstar;
rstar        =         rhorstar*rstar(-1) + (1 - rhorstar)*(kappapistar*piYstar + kappaystar*(ystar - ystar(-1))) + etaRstar;  // Foreign Policy Rule
...
E    =   betta/(1+betta)*E(+1) + 1/(1+betta)*E(-1)
                     + (1-betta*xiE)*(1-xiE)/((1+betta)*xiE)*(N - E);
...
%(64) Output
dy_             =  y - y(-1) + gz + log(gzbar)*100; // + me_y;

%(65) Consumption
dc_             =   c - c(-1) + gz + log(gzbar+0.0014)*100; // + me_c;

%(66) Government consumption
dg_             =   g - g(-1) + gz + log(gzbar-0.0006)*100; // + me_g;

%(66) Government debt
db_             =   b - b(-1) + gz + log(gzbar+0.0049)*100 + me_b; //   Actual is different: 1.194766034 % CHECK all.

%(67) Exports
dx_             =   x - x(-1) + gz + log(gzbar+0.0002)*100; // + me_x;
%dx_             =  - mustar*(  pX - pY - s ) + ystar - ystar(-1) + gz + log(gzbar+0.0002)*100; // + me_x;
 
%(68) Imports
dim_            =   im - im(-1) + gz + log(gzbar+0.0051)*100; // + me_im;

%(69) Employment
dE_             =   E - E(-1) + log(1.0015)*100; // + me_E;

%(70) Real wages
dw_             =   w - w(-1) + gz + log(1.0049)*100; // + me_w;

%(71) Nominal interest rate
R_              =   r + 100*log(Rbar);

%(72) Private investment
di_             =   i - i(-1) + gz + log(gzbar+0.0042)*100; //  + me_i;

%(73) Government investment
diG_            =   iG - iG(-1) + gz + log(gzbar+0.0038)*100; // + me_ig;

%(74) Transfers
dtr_            =   tr - tr(-1) + gz + log(gzbar+0.0093)*100; // + me_tr;

%(75) Import inflation
pi_im_          =   piIM + log(Pibar)*100; // + me_pi_im; %% error: was piI should be pi_IM %% PiIMbar?

%(75) Export inflation
pi_x_          =   piX + log(Pibar)*100; // + me_pi_x; %% included since params estimated

%(76) Consumption deflator
pi_c_           =   piC + log(Pibar)*100; // %% PiCbar? or assumed all inflation at same level in data?

%(77) Inflation target
pi_cbar_        =   piCbar + log(Pibar)*100; 

%(78) GDP deflator / PPI (domestic phillips curve) %% pi_h_ = PPI ; pi_y_ = GDP deflator piY ...
%pi_y_           =   piY + log(Pibar)*100; // + me_pi_y;  
pi_h_           =   piH + log(Pibar)*100; // + me_pi_h;  

%(79)
dtauC_          =   tauC - tauC(-1) + 100*log(1.0001); // + me_tauC;

%(80)
dtauN_          =   tauN - tauN(-1) + 100*log(1.0004); // + me_tauN;

%(81)
dtauK_          =   tauK - tauK(-1) + 100*log(1.0008); // + me_tauK;

%(82) Foreign output
dy_star_        =    ystar - ystar(-1) + gz + log(gzbar+0.0004)*100; // % + me_ystar; // + ztilde - ztilde(-1) 

%(83) Foreign interest rate
R_star_         =    rstar + 100*log(Rstarbar);

%(84) Foreign inflation
pi_Ystar_       =   piYstar + 100*log(piYstarbar);

%(85) Real (Error: previously Nominal) exchange rate
dsr_ =   s - s(-1) + 100*log(gzbar+0.0073); // + me_s; // + piY - piYstar + 100*log(Pibar) - 100*log(piYstarbar)
...
%steady;
SAFiscal_steadystate([],[],1); %print steady state and parameters
steady;
...
//var me_y;       stderr	sqrt(0.3565113557/(100/15))         ;	
//var me_c;       stderr	sqrt(0.4928579668/(100/15))         ;
//var me_w;       stderr	sqrt(2.6396945775/(100/15))         ;	
//var me_E;       stderr	sqrt(0.4677243071/(100/15))         ;
//var me_i;       stderr	sqrt(6.1015204344/(100/15))         ;
//var me_x;       stderr	sqrt(25.3794811631/(100/15))         ;
//var me_im;      stderr	sqrt(13.9154776656/(100/15))         ;
//var me_pi_im;    stderr	sqrt(1.3913173421/(100/15))         ;
//var me_pi_x;    stderr	sqrt(1.3913173421/(100/15))         ;  // CHECK
//var me_pi_h;    stderr	sqrt(1.5888635216/(100/15))         ;

var me_b;       stderr	sqrt(5.563550628/(100/10))         ;
//var me_g;       stderr	sqrt(0.8064222755/(100/15))         ;
//var me_ig;      stderr	sqrt(18.4004932754/(100/15))         ;
//var me_tr;      stderr	sqrt(65.5705740993/(100/15))         ;
//var me_tauN;    stderr	sqrt(1.1589729086/(100/15))         ;
//var me_tauK;    stderr	sqrt(5.3070449191/(100/15))         ;	
//var me_tauC;    stderr	sqrt(1.5280035865/(100/15))         ;
////var me_s;       stderr	sqrt(4.862528236/(100/15))         ;	

var etag;      stderr   0.1;      
var etaiG;          stderr   0.1; 
var etatr;           stderr   0.1; 
var etatauC;         stderr   0.1;
var etatauN;         stderr   0.1;
var etatauK;         stderr   0.1;
var etaYstar;        stderr   0.1;
var etaPiYstar;      stderr   0;  // 0.1
var etaRstar;        stderr   0.1;
var etagz;        stderr   0;     // 0.1
var etaepsilon;    stderr   0.1;  
var etaI;           stderr   0.1;
var etavarphiH;      stderr   0.1;
var etavarphiX;   stderr   0.1;   
var etavarphistar;   stderr   0.1;
var etavarphiW;    stderr   0; 
var etaR;      stderr   0.1;
var etaRPG;   stderr   0.1;       
var etaRPstar;  stderr   0.1;
var etapiCbar;       stderr   0.1;
...
% Ricardian shares
//omega,      beta_pdf, 0.5, 0.1;          % Share of non-Ricardian
...
//nuG,        gamma_pdf, 1.0, 0.2;            % Consumption CES       
//nuK,        gamma_pdf, 1.0, 0.2;            % Capital CES       
...
thetastar,  beta_pdf, 0.8, 0.05;            % Calvo: Import prices
chistar,    beta_pdf, 0.75, 0.1;            % Indexation: Import prices

% Adjustment costs
gammaI,     gamma_pdf, 8.0, 1.5;            % Investment

% Monetary policy
phiR,       beta_pdf, 0.8, 0.05;            % Interest rate smoothing
phiPi,      normal_pdf, 1.7, 0.1;           % Resp. to inflation
phiY,       normal_pdf, 0.25, 0.05;         % Resp. to output
phiDPi,     normal_pdf, 0.3, 0.1;           % Resp. to change in inflation
phiDY,      normal_pdf, 0.125, 0.05;        % Resp. to change in output
...
rhoepsG,        beta_pdf, 0.85, 0.1; 
rhoepsiG,       beta_pdf, 0.85, 0.1; 
rhoepstr,       beta_pdf, 0.85, 0.1;
rhoepstauC,     beta_pdf, 0.85, 0.1;
rhoepstauN,     beta_pdf, 0.85, 0.1;
rhoepstauK,     beta_pdf, 0.85, 0.1;

% Employment
//xiE,        beta_pdf, 0.5, 0.15;                % Calvo style parameter

% Autoregressive coefficients

rhoRPG,         beta_pdf, 0.85, 0.1;            % Domestic risk premium
rhoRPstar,      beta_pdf, 0.85, 0.1;            % Foreign risk premium
//rhogz,          beta_pdf, 0.85, 0.1;            % Permanent tech. shock
rhoepsilon,     beta_pdf, 0.85, 0.1;            % Transitory tech. shock
rhoN,           beta_pdf, 0.85, 0.1;            % Labour shock
rhoI,           beta_pdf, 0.85, 0.1;            % Investment-specific tech. shock
//rhovarphiW,     beta_pdf, 0.85, 0.1;            % Wage markup shock
rhovarphiH,     beta_pdf, 0.85, 0.1;            % Domestic price markup shock
rhovarphiX,     beta_pdf, 0.85, 0.1;            % Export price markup shock
rhovarphistar,  beta_pdf, 0.85, 0.1;            % Import price markup shock
rhog,           beta_pdf, 0.85, 0.1;           % Gvt. spending shock
rhoiG,          beta_pdf, 0.85, 0.1;           % Gvt. investment shock
rhotr,          beta_pdf, 0.85, 0.1;           % Transfer shock
rhotauC,        beta_pdf, 0.85, 0.1;           % Consumption tax shock
rhotauN,        beta_pdf, 0.85, 0.1;           % Labour tax shock
rhotauK,        beta_pdf, 0.85, 0.1;           % Capital tax shock

% Structural shocks

stderr etaYstar,        inv_gamma_pdf, 0.1, inf;
//stderr etaPiYstar,      inv_gamma_pdf, 0.1, inf;
stderr etaRstar,        inv_gamma_pdf, 0.1, inf;
stderr etag,            inv_gamma_pdf, 0.1, inf;
stderr etaiG,           inv_gamma_pdf, 0.1, inf;
stderr etatr,           inv_gamma_pdf, 0.1, inf;
stderr etaR,            inv_gamma_pdf, 0.1, inf;
stderr etaRPG,          inv_gamma_pdf, 0.1, inf;
stderr etaRPstar,       inv_gamma_pdf, 0.1, inf;
//stderr etagz,           inv_gamma_pdf, 0.1, inf;
stderr etaepsilon,      inv_gamma_pdf, 0.1, inf;
stderr etaI,            inv_gamma_pdf, 0.1, inf;
stderr etavarphiH,      inv_gamma_pdf, 0.1, inf;
stderr etavarphiX,      inv_gamma_pdf, 0.1, inf;
stderr etavarphistar,   inv_gamma_pdf, 0.1, inf;
//stderr etavarphiW,      inv_gamma_pdf, 0.1, inf;
stderr etatauC,         inv_gamma_pdf, 0.1, inf;
stderr etatauN,         inv_gamma_pdf, 0.1, inf;
stderr etatauK,         inv_gamma_pdf, 0.1, inf;
stderr etapiCbar,       inv_gamma_pdf, 0.1, inf;
...
dy_  R_  pi_c_  pi_h_ db_ dg_ diG_ dtr_  dtauN_ dtauK_ dtauC_   dy_star_ R_star_ pi_Ystar_  pi_cbar_; //  dc_  di_  dE_   dx_ dim_ dsr_    pi_im_ pi_x_	  dw_ 

%--------------------------------------------------------------------------
options_.plot_priors=0; // mode_file=SAFiscal_mh_mode, load_mh_file,   mode_check, 
// estimation(order=1,   mode_file=SAFiscal_mh_mode,    
// paste in model command line to check collinearity issues: model_diagnostics(M_,options_,oo_) 
// options_.debug=1; % print_info.m 
%--------------------------------------------------------------------------

estimation(datafile=data_18q4_V2, mh_nblocks=2, mh_replic=100000, mh_drop=0.5, mh_jscale=0.25, 
           mode_compute=4, mode_file=SAFiscal_mode, bayesian_irf, irf=20, moments_varendo, conditional_variance_decomposition=[1:20],
           graph_format=fig) b y r g iG dy_	dg_ diG_ c cI cJ i x im piY piC piIM N piW s; // 

// Shock Decomposition option

shock_groups(name=groups);
'Technology and Labour' = etagz etaepsilon etaN;
...
'Foreign' = etaYstar etaPiYstar etaRstar etaRPstar;
end;

//// Shock Decomposition option
// shock_decomposition(colormap=jet) b y r g;    // rr_obs
// plot_shock_decomposition(graph_format=(fig), use_shock_groups=groups, colormap=jet, steadystate) b y r g;
