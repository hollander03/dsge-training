...
var m v p y r int int_T pii ytilde yn rn rmc h fr xi_z xi_ms n xi_md xi_fr; // xi_v 

varexo epsil_z epsil_i epsil_ms epsil_md epsil_fr;  // epsil_v 

parameters alppha betta eta_c eta_m eta_n theta_p
           nu_h nu_fr rho_h rho_fr FR RR s
           rho_z rho_i rho_ms rho_md rho_frs 
...
eta_c = 3.635;   // 
eta_m = 4.92;    // see Walsh
eta_n = 0.98;    // Inverse of the Frisch elasticity of labour supply
theta_p = 0.873; // price stickiness
//nss = 0.3333;  // steady state labour 1/3 p. 71 Walsh.

rho_h = 0.437;   //  nonborrowed reserve persistence 
rho_fr = 0.201;  //  free reserves shock
nu_h = 11.374;   // 11.374 elasticity of nonborrowed reserves to nominal interest rate 100
nu_fr = 12.239;  // 12.239 elasticity of free reserves to nominal interest rate        500
...
rho_z = 0.763;   //  tech shock
rho_frs = 0.75;   // free reserve shock
rho_ms = 0.77;   //  money (reserve) supply shock
...
var epsil_z; stderr 0.038; // var(epsil) = 0.01^2
//var epsil_fr; stderr 0.0025; // var(epsil) = 0.01^2
var epsil_i; stderr 0.005; // var(epsil) = 0.01^2
var epsil_ms; stderr 0.079; // var(epsil) = 0.01^2
var epsil_md; stderr 0.018; // var(epsil) = 0.01^2
...
stoch_simul(order=1, irf=0, graph_format=fig); // periods=10000,

forecast y pii int m v int_T;
...
conditional_forecast(parameter_set = calibration, controlled_varexo = (epsil_i), replic = 5000, periods=40);

plot_conditional_forecast(periods = 40) y pii int m v int_T;
