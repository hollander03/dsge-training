...
Q                   $\hat{Q}$                   % Tobin's Q
s                   $\hat{s}$                   % Real (error: declared as Nominal) exchange rate [CHECK: lower case s declared as real in appendix p. 52, Eq.A6]
...
%%%%% estimated params fixed
 ...
 thetaW	=	0.483006426	;
 chiW	=	0.512497013	;
 gammaB	=	0.038477396 ; // 0 // 0.038477396;
 phiR	=	0.871525583	; // 0.99; // 
 phiPi	=	1.567210076	; // 1.1; // 
 phiDY	=	0.387040673	; // 0; // 
...
sIG	=	0.057	;
sTR	=	0.042	;
sBbar	=	2	;  % not the ratio reflects the stock over *quarterly* flow and therefore SS ratio multiplied by 4
...
%(2) Investment FOC // double derivation of log-linearized equation (A.2)/(A.3)
...
R_/100              =   r + log(Rbar);  // lRbar = 0.023838413641444//  Rbar = 1.02416772929
...
var	etag;	stderr	0.01; // 0.00619333	; 0.006204983686847
...
stoch_simul(irf=20, graph_format=fig, nodisplay) b y r g iG tr gdebtgdp_ db_ dy_ R_ dg_ diG_ dtr_ rrp rrpstar susgap susgapgdp pbudsus pbud dy piY; // c cI cJ i x im piY piC piIM N piW s;
