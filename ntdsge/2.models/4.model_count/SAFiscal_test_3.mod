...
alphaK_target       =   0.9;
cJcISSratio         =   0.8;
tr2rot              =   0.46; %// calibrated to DSGE paper est
sIMC                =   0.224;    
sIMI                =   0.06;
sI                  =   0.126;
sG                  =   0.195;
...
%steady;
SAFiscal_steadystate_3([],[],1); %print steady state and parameters
steady;
...
dy_   dE_  pi_c_  dw_   R_                  //   NYFED obs plus dw_ obs 5 // other for SW obs:   dc_  di_     
...
estimation(datafile=data_18q4_V2, load_mh_file, mh_replic=0, mh_nblocks=3, // mh_replic=200000, mh_drop=0.5, mh_jscale=0.2, 
           mode_compute=0, mode_file=SAFiscal_test_3_mh_mode, mode_check,                      // mode_file=Copy_2_of_SAFiscal_mode, (latest), Copy_9_of_SAFiscal_mode
           bayesian_irf, irf=20, moments_varendo, conditional_variance_decomposition=[1:20],
           graph_format=fig) b y r g iG tr gdebtgdp_ db_ dy_ R_ dg_ diG_ dtr_ rrp rrpstar c cI cJ i x im piY piC piIM N piW s susgapgdp by pbud pbudsus susgap; //  
...
//// Shock Decomposition option
 ...
 plot_shock_decomposition(graph_format=(fig), use_shock_groups=groups, colormap=jet, steadystate) gdebtgdp_ rrp b y r g by susgapgdp;
