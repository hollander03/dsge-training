...
if isempty(y0)==1 || good_steady==0
    y0 = [log(45);               % k    %transformed on real line, para>0
        ...
        log(1.30);              % cJ
        log(1.30);              % cI
        log(1.30);                    % trJ -- logged - prevent negative val - previous 0.3
        log(1.30);                    % trI -- previous 0.3
        0.2;                     % tauC
...
if exitflag~=1 || isreal(y)~=1, %try again
    ...
    y0 = [log(45);               % k    %transformed on real line, para>0
        ...
        log( 0.30);              % cJ
        log( 0.30);              % cI
        log(1.10);                    % trJ -- logged - prevent negative val - previous 0.3
        log(1.10);                    % trI - previous 0.3
        0.2;                     % tauC
        log( 0.8/(1-0.8));       % alphaK
        log( 0.8/(1-0.8));       % alphaG
        0];
...
out_ss = [exp(y( 1));            % k
    ...
    exp(y( 13));                 % cJ
    exp(y( 14));                 % cI
    (y(15));                       % trJ %% exp added because of log trans - prevent negative val
    (y(16));                       % trI %% exp added because of log trans
    y(17);                       % tauC
