...
rho_z = 0.75;     //  tech shock
//rho_p = 0.9;     //  price markup shock
rho_ms = 0.75;    //  money (reserve) supply shock
rho_md = 0.75;    //  money demand shock
...
%%% 2001-2022
ytilde_ss =  0.0039937;   // -0.00759341
pii_ss =  0.0637/4;      // 0.024899295
int_ss = 0.0726/4;       // 0.049106316
m_ss = 0.0127;              // Div M4 qq   // Div M4 yy 0.04481255385396 (1984-2007)
...
ytilde_obs/100 = ytilde + ytilde_ss;
pii_obs/400 = pii + pii_ss;
int_obs/400 = int + int_ss;
m_obs/100 = m - m(-1) + m_ss;
...
estimated_params;
  ...
  %%coefficients of AR(1) shocks
    rho_z, beta_pdf, 0.75, 0.1;           // productivity shock persistance parameter
    //rho_p, beta_pdf, 0.5, 0.1;           // money demand shock !!!!!
    rho_i, beta_pdf, 0.75, 0.1;           // MP policy shock smoothing
    rho_ms, beta_pdf, 0.75, 0.1;          // money (reserve) supply shock
    rho_md, beta_pdf, 0.75, 0.1;           //  effective reserve requirement (money demand) shock --- see Eq. 23, p. 16

  %%innovations of the 5 shocks
    stderr epsil_z, inv_gamma_pdf, 0.02, inf;            // (1)  productivity shock
    //stderr epsil_p, inv_gamma_pdf, 0.02, inf;            // (2) money demand shock !!!!!
    stderr epsil_i, inv_gamma_pdf, 0.02, inf;            // (3) policy rate rule shock
    stderr epsil_ms, inv_gamma_pdf, 0.02, inf;           // (4) money (reserve) supply shock
    stderr epsil_md, inv_gamma_pdf, 0.02, inf;           // (5) money (reserve) demand shock  
...
estimation(datafile=data_M, mh_replic=100000, mh_drop=0.5, mh_nblocks=2, mh_jscale=0.425,bayesian_irf, irf=20, // load_mh_file, mode_check, 
           plot_priors=0, mode_compute=0, mode_file=NK_pre_crisis_mode, 
           moments_varendo, conditional_variance_decomposition=[1:20],
...
//// Shock Decomposition option
 shock_decomposition(colormap=jet) m ;    // rr_obs v p y int_obs pii_obs r ytilde_obs m_obs
