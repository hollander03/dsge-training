...
function out_ss = SAFiscal_ss(beta, sigmaL, kappa, gz, alpha, varphiH, varphiW, muC, muI, sI, sG, sIMC, ...
...
if isempty(y0)==1 || good_steady==0
    y0 = [log(45);               % k    %transformed on real line, para>0
        ...
        log( 0.30);              % cJ
        log( 0.30);              % cI
        0.3;                    % trJ -- logged - prevent negative val - previous 0.3 // CHECK: needed for BIG model counterfactuals log(1.10)
        0.3;                    % trI -- previous 0.3 // CHECK: needed for BIG model counterfactuals log(1.10)
        0.2;                     % tauC
...
[y,fval,exitflag,output] = fsolve('SAFiscal_ss_equations',y0,options,...
    beta, sigmaL, kappa, gz, alpha, varphiH, varphiW, muC, muI, sI, sG,...
...
if exitflag~=1 || isreal(y)~=1, %try again
    ...
    y0 = [log(45);               % k    %transformed on real line, para>0
        ...
        0.3;                    % trJ -- logged - prevent negative val - previous 0.3 // needed for BIG model counterfactuals log(1.10)
        0.3;                    % trI - previous 0.3 // needed for BIG model counterfactuals log(1.10)
        0.2;                     % tauC
        log( 0.8/(1-0.8));       % alphaK
        log( 0.8/(1-0.8));       % alphaG
        0];
...
out_ss = [exp(y( 1));            % k
    ...
    exp(y( 13));                 % cJ
    exp(y( 14));                 % cI
    y(15);                       % trJ %% exp added because of log trans - prevent negative val // exp(y(15))
    y(16);                       % trI %% exp added because of log trans - prevent negative val // exp(y(16))
    y(17);                       % tauC
