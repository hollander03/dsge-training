...
% Fiscal variables
bbar              = sBbar*pYbar*ybar; %% what is sBbar? G.Debt/Y... sBbar calibrated to 2 ... implied ss reasonable based on this but check implications throughout
...
trYbar              =   trbar/(pYbar*ybar); % equals calibrated sTR
...
% Foreign inflation and interest rate % NOTE: ss Rstarbar only relevant for
% NFA eqn 
PiYstarbar           =   1.00549514; % 1.00549513942308
Rstarbar             =   (PiYstarbar*gzbar)/betastar; % 1.010826176; %(PiYstarbar*gzbar)/betastar; // 1.00607006931005
...
if check==0,%Dynare Switch indicating good steady state
    ...
    pbud=0; pbudsus=0; susgap=0; susgapgdp=0; credratefull_=0; credratenan_=0; epsiloncred=0;
    ...
    dy_=log(gzbar)*100;	
    dc_=log(1.0078755167)*100;
    dg_=log(1.005512837)*100;
    db_=log(1.01224346)*100; 
    gdebtgdpdt_ = 0;
    gdebtgdp_ = (36.607);
    by = 0;
    bstary = 0;
    dx_=log(1.005844978)*100;		
    dim_=log(1.0110935565)*100;    
    dE_=log(1.001702712)*100;		
    dw_ = log(1.00486)*100;
    R_=100*log(Rbar);			
    di_=log(1.0098607)*100;		
    diG_=log(1.00504773)*100;	
    dtr_=log(1.0121173)*100;    
    pi_im_=log(Pibar)*100;
    pi_x_=log(Pibar)*100;
    pi_c_=log(Pibar)*100;
    pi_cbar_=log(Pibar)*100;
    pi_h_=log(Pibar)*100; 
    dtauC_=100*log(1.000115);
    dtauN_=100*log(1.001309);  
    dtauK_=100*log(1.00067);  
    dy_star_=log(1.00618)*100;
    R_star_=100*log(1.00607);	
    pi_Ystar_=100*log(piYstarbar);
    dsr_=100*log(1.0021162); %  + 100*log(Pibar) - 100*log(piYstarbar)
