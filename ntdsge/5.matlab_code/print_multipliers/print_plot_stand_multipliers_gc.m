...
% h=size(oo_.irfs.y_etag,2);
h=size(oo_.PosteriorIRF.dsge.Mean.y_etag);  
...
gbarbase=M_.params(loc(M_.param_names(:,:),'gbar'));

% resp_mat_fis(1,:)=oo_.irfs.g_etag(:,1:h);
% resp_mat_y(1,:)=oo_.irfs.y_etag(:,1:h);
% resp_mat_c(1,:)=oo_.irfs.c_etag(:,1:h);
% resp_mat_i(1,:)=oo_.irfs.i_etag(:,1:h);

resp_mat_fis(1,:)=oo_.PosteriorIRF.dsge.Mean.g_etag(:,1);
resp_mat_y(1,:)=oo_.PosteriorIRF.dsge.Mean.y_etag(:,1);
resp_mat_c(1,:)=oo_.PosteriorIRF.dsge.Mean.c_etag(:,1);
resp_mat_i(1,:)=oo_.PosteriorIRF.dsge.Mean.i_etag(:,1);
...
mult_cum_y=cumsum(ybarbase.*resp_mat_y,2)./cumsum(gbarbase.*resp_mat_fis,2);
mult_pv_y=cumsum(disc_mat.*ybarbase.*resp_mat_y,2)./cumsum(disc_mat.*gbarbase.*resp_mat_fis,2);

mult_cum_c=cumsum(cbarbase.*resp_mat_c,2)./cumsum(gbarbase.*resp_mat_fis,2);
mult_pv_c=cumsum(disc_mat.*cbarbase.*resp_mat_c,2)./cumsum(disc_mat.*gbarbase.*resp_mat_fis,2);

mult_cum_i=cumsum(ibarbase.*resp_mat_i,2)./cumsum(gbarbase.*resp_mat_fis,2);
mult_pv_i=cumsum(disc_mat.*ibarbase.*resp_mat_i,2)./cumsum(disc_mat.*gbarbase.*resp_mat_fis,2);
