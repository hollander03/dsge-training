...
taubase=M_.params(loc(M_.param_names(:,:),'cbar'));
tauCbase=(M_.params(loc(M_.param_names(:,:),'tauCbar')))*M_.params(loc(M_.param_names(:,:),'cbar'));

resp_mat_fis(1,:)=oo_.irfs.tauC_etatauC(:,1:100);
resp_mat_y(1,:)=oo_.irfs.y_etatauC(:,1:100);
resp_mat_c(1,:)=oo_.irfs.c_etatauC(:,1:100);
resp_mat_i(1,:)=oo_.irfs.i_etatauC(:,1:100);
...
mult_cum_y=cumsum(ybarbase.*resp_mat_y,2)./cumsum(tauCbase.*taubase.*resp_mat_fis,2);
mult_pv_y=cumsum(disc_mat.*ybarbase.*resp_mat_y,2)./cumsum(disc_mat.*tauCbase.*taubase.*resp_mat_fis,2);

mult_cum_c=cumsum(cbarbase.*resp_mat_c,2)./cumsum(tauCbase.*taubase.*resp_mat_fis,2);
mult_pv_c=cumsum(disc_mat.*cbarbase.*resp_mat_c,2)./cumsum(disc_mat.*tauCbase.*taubase.*resp_mat_fis,2);

mult_cum_i=cumsum(ibarbase.*resp_mat_i,2)./cumsum(tauCbase.*taubase.*resp_mat_fis,2);
mult_pv_i=cumsum(disc_mat.*ibarbase.*resp_mat_i,2)./cumsum(disc_mat.*tauCbase.*taubase.*resp_mat_fis,2);
