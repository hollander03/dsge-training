...
//kappa_v*(v - k) = kappa_v*beta_h*(v(+1) - k(+1)) + (1 - beta_h*(1 - delt))*(r_k(+1)) + (sigma_c)*(c - c(+1)); // + (sigma_c/(1-phi))*((c - phi*c(-1)) - (c(+1) - phi*c)); // Investment schedule
...
%%%% note: above timing of investment schedule adjusted due to countercyclical investment. See article for reasons habit formation excluded in the stochastic discount factor 
...
mc_h = r_k - (y_h - nu*k - yx);                        // with vartheta not equal to 1 real marginal cost wrt FOC capital
mc_h = (pr_o - pr_h) - (y_h - nu*oil_h - yx);          // with vartheta not equal to 1 real marginal cost wrt FOC oil
yx = vtheta*(1 - nu)*k + (1 - vtheta)*(1 - nu)*oil_h;  // yx is the nested term in prod func with elastic subst btw oil and capital

y_h = a + alph*n + (1 - alph)*vtheta*k + (1 - alph)*(1 - vtheta)*oil_h; // Cobb-Douglas production function with oil
...
estimation(datafile=GHWEstimation7, mh_nblocks=3, mh_replic=100000, mh_drop=0.5, mh_jscale=0.26,
           mode_compute=0, mode_file=GHW2017b_SOEoil_mode, mode_check, bayesian_irf, irf=20, moments_varendo, conditional_variance_decomposition=[1:20],
           graph_format=fig) y pi_tot int_b pi_home pi_foreign pi_oil rer e_dot oil y_star pi_star int_b_star oil_c oil_h v k n;
